...
export default function NewAgentScreen() {
  ...
  const { preset: presetParam, projectId: projectIdParam } = useLocalSearchParams<{ preset?: string; projectId?: string }>();
  ...
  // ── Auto-apply preset from query param ──────────────────────────────────────
  useEffect(() => {
    if (presetParam) {
  ...
  async function handleAiSave() {
    ...
    try {
      ...
      if (projectIdParam) {
        router.replace(`/project/${projectIdParam}` as any);
      } else {
