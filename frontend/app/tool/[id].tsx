  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        {id === 'convert' && (
          <View style={{ marginBottom: SPACING.lg }}>
            <Text style={styles.fieldLabel}>PIATTAFORMA / FORMATO</Text>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: SPACING.sm }}>
              <View style={[styles.modeToggle, { flexWrap: 'nowrap' }]}>
                {[
                  { label: 'MP3', value: 'mp3' },
                  { label: 'WAV', value: 'wav' },
                  { label: 'MP4', value: 'mp4' },
                  { label: 'WebM', value: 'webm' },
                  { label: '🎬 YouTube 1080p', value: 'youtube_1080p' },
                  { label: '🎬 YouTube 4K', value: 'youtube_4k' },
                  { label: '📱 TikTok', value: 'tiktok' },
                  { label: '📱 Reels', value: 'reels' },
                  { label: '📱 Shorts', value: 'shorts' },
                  { label: '🎙️ Podcast', value: 'podcast' },
                ].map((item) => (
                  <Pressable
                    key={item.value}
                    onPress={() => setConvertFormat(item.value)}
                    style={[styles.modeBtn, convertFormat === item.value && styles.modeBtnActive]}
                  >
                    <Text style={[styles.modeBtnText, convertFormat === item.value && styles.modeBtnTextActive]}>
                      {item.label}
                    </Text>
                  </Pressable>
                ))}
            ...
            <GlowCard gradient={tool.gradient} glowIntensity={0.12}>
              {id === 'tts' ? (
                <View>
                  ...
                  <WaveformVisualizer playing height={48} color={COLORS.neonCyan} style={{ marginTop: SPACING.md }} />
                </View>
              ) : id === 'convert' ? (
