# ============================================================
#  Creator Suite - Local startup (Windows PowerShell)
#  Usage: .\start.ps1
#         .\start.ps1 -SkipInstall   (if deps are already installed)
# ============================================================
param(
    [switch]$SkipInstall
)
...
if (-not (Test-Path $envFile)) {
    ...
    $envContent = $envContent -replace '^DATABASE_URL=postgresql.*', 'DATABASE_URL=sqlite+aiosqlite:///./creator_suite_dev.db'
...
Write-Host "  Login        -> use the credentials from your backend/.env (DEV_EMAIL / DEV_PASSWORD)"
Write-Host ""
