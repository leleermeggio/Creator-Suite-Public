...
@pytest_asyncio.fixture
async def app(tmp_path, monkeypatch):  # noqa: PT004
    ...
    # Mock Celery so routes don't require a live Redis/broker
    mock_task = MagicMock()
    mock_task.delay = MagicMock(return_value=MagicMock(id="test-task-id"))
    monkeypatch.setattr("backend.workers.tasks.process_job", mock_task)
