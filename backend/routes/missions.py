...
@router.post(
    "/{mission_id}/steps/{step_index}/complete", response_model=MissionResponse
)
async def complete_step_route(
    mission_id: str,
    step_index: int,
    body: StepCompleteBody | None = None,
    user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    from backend.services.mission_service import complete_step as svc_complete

    try:
        output = body.output if body else None
        return await svc_complete(db, mission_id, step_index, user.id, output)
    except ValueError as exc:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail=str(exc)
        )
