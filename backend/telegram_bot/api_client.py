...
class CreatorSuiteClient:
    ...
    async def get_upload_url(
        ...
        return await self._request("POST", "/media/upload-url", json={
            "project_id": project_id, "filename": filename,
            "content_type": content_type, "size_bytes": size_bytes,
        })

    async def register_asset(
        self, project_id: str, filename: str, storage_key: str,
        mime_type: str, size_bytes: int, duration_seconds: float | None = None,
    ) -> dict:
        return await self._request("POST", "/media/register", json={
            "project_id": project_id, "filename": filename,
    ...
    async def import_url(self, project_id: str, url: str) -> dict:
        return await self._request("POST", "/media/import-url", json={
    ...
    async def create_export(
        ...
        return await self._request("POST", "/exports/", json={
            "project_id": project_id, "format_preset": format_preset, **kwargs,
        })
    ...
    async def generate_captions(
        self, project_id: str, asset_id: str | None = None, language: str = "auto",
    ) -> dict:
        return await self._request("POST", "/captions/generate", json={
            "project_id": project_id, "asset_id": asset_id, "language": language,
        })
    ...
    async def translate_caption(self, caption_id: str, target_language: str) -> dict:
        return await self._request("POST", f"/captions/{caption_id}/translate", json={
    ...
    async def search(self, project_id: str, query: str) -> list[dict]:
        return await self._request("POST", "/search/", json={
            "project_id": project_id, "query": query,
        })

    # --- Graphics Overlays ---

    async def create_overlay(self, project_id: str, overlay_type: str, **kwargs) -> dict:
        return await self._request("POST", "/overlays/", json={
            "project_id": project_id, "overlay_type": overlay_type, **kwargs,
        })
    ...
    async def delete_overlay(self, overlay_id: str) -> None:
        client = await self._get_client()
        resp = await client.request("DELETE", f"/overlays/{overlay_id}", headers=self._headers)
        resp.raise_for_status()

    # --- Watermark ---

    async def add_image_watermark(
        self, project_id: str, asset_id: str, watermark_storage_key: str, **kwargs,
    ) -> dict:
        return await self._request("POST", "/watermark/image", json={
            "project_id": project_id, "asset_id": asset_id,
            "watermark_storage_key": watermark_storage_key, **kwargs,
        })

    async def add_text_watermark(
        self, project_id: str, asset_id: str, text: str, **kwargs,
    ) -> dict:
        return await self._request("POST", "/watermark/text", json={
            "project_id": project_id, "asset_id": asset_id, "text": text, **kwargs,
        })
